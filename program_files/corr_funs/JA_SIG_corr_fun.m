function JA_SIG_Cvals = JA_SIG_corr_fun(sol_data_set,sigma)
...
% this variable  must be formatted this way for this function to work!
    ...
    %!!! Sc cannot be set here, only log10(Sc) = (log10(Sp) + sigma) can be
    %defined
    Sp = sol_data_set{1,3};
    ...
    for i = 1:height(sol_data_set)
        ...
        reg_table(i,6) = log10(Sm) - fc*(log10(Sp)-sigma) - fp*log10(Sp);
    end
    ...
    JA_SIG_Cvals = regress(reg_table(:,6),[reg_table(:,3),reg_table(:,4),reg_table(:,5)]);
